...
if (isset($login_session)) {
    if (strpos($login_session, 'stefano-') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'stefano.mp3'];
    }
    // Add more users as needed
    if (strpos($login_session, 'kelvin-') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'kelvin.mp3'];
    }

    if (strpos($login_session, 'cockman') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'cockman.mp3'];
    }

    if (strpos($login_session, 'joseph') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'joe.mp3'];
    }

    if (strpos($login_session, 'jeffrey') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'jeff.mp3'];
    }

    if (strpos($login_session, 'veronica') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'veronica.mp3'];
    }

    if (strpos($login_session, 'alejandro') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'ale.mp3'];
    }

    if (strpos($login_session, 'katherine') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'kat.mp3'];
    }

    if (strpos($login_session, 'ugo-') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'ugo.mp3'];
    }

    if (strpos($login_session, 'morales') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'morales.mp3'];
    }

    if (strpos($login_session, 'megan') === 0) {
        $extraNarrations[] = ['path' => 'narr', 'file' => 'megan.mp3'];
    }
}
...
const EXTRA_NARRATIONS = <?php echo json_encode($extraNarrations); ?>;
...
<?php
$initialY = 330;
echo "const initialShipY = $initialY;";
?>
        class Game {
            constructor() {
...
if (currentNarration && !currentNarration.paused) {
    try {
        ...
        console.warn("⚠️ Could not pause narration:", err);
    }
}
clearInterval(narrationInterval);
currentNarration = null;

// 🚫 Mark that we're in death narration mode
narrationPausedDueToDeath = true;

if (currentDeathNarration && !currentDeathNarration.paused) {
    try {
        currentDeathNarration.pause();
        currentDeathNarration.currentTime = 0;
    } catch (err) {
        console.warn("⚠️ Could not pause previous death narration:", err);
    }
}
currentDeathNarration = null;

const deathAudio = deathNarrations[Math.floor(Math.random() * deathNarrations.length)];
currentDeathNarration = deathAudio;

deathAudio.onended = () => {
    currentDeathNarration = null;
    narrationPausedDueToDeath = false; // ✅ Now allow normal narration again
};

setTimeout(() => {
    deathAudio.volume = 1; // Force max volume
    deathAudio.currentTime = 0; // Optional: restart from beginning just in case
...
reset() {
    ...
    setTimeout(() => {
    restoreMusicSpeedSmoothly();
}, 200); // small delay helps avoid race conditions
    ...
    this.spaceship.style.opacity = this.invisibleActivated ? '0' : '1';
    this.spaceship.style.top = this.shipY + 'px';
    ...
    this.setShipTransform({ rotate: 0, scale: this.bigSpriteActivated ? 2 : 1 });
